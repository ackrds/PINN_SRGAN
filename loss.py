...
class NSLoss(torch.nn.Module):
    def __init__(self, loss, diff, rho):
    ...
    def forward(self, preds):
        ...
        u_x = field_grad(u,0)
        u_xx = field_grad(u_x,0)
        u_y = field_grad(u,1)
        u_yy = field_grad(u_y,1)
        v_x = field_grad(v,0)
        v_xx = field_grad(v_x,0)
        v_y = field_grad(v,1)
        v_yy = field_grad(v_y,1)
        
        p_x = field_grad(p,0)
        p_y = field_grad(p,1)
        loss_pred_1 = u*v_x + v*v_y - self.diff*(v_xx+  v_yy) + 1/self.rho*(P_y) 
        loss_pred_2 = u*v_x + v*v_y - self.diff*(v_xx+  v_yy) + 1/self.rho*(P_y) 
        loss_pred_3 = u_x + v_y
        ...
        # p_x = field_grad(p,0)
        # p_y = field_grad(p,1)
        # loss_true_1 = u*v_x + v*v_y - seld.diff*(v_xx+  v_yy) + 1/self.rho*(P_y) 
        # loss_true_2 = u*v_x + v*v_y - self.diff*(v_xx+  v_yy) + 1/self.rho*(P_y) 
        # loss_true_3 = u_x + v_y
        # loss_true = loss_true_1 + loss_true_2 + loss_true_3 
        
        return self.loss(loss_pred, 0*loss_pred)
